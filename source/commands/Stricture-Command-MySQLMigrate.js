/**
 * Stricture CLI Command - MySQL Migrate
 *
 * Generates MySQL INSERT INTO ... SELECT migration stubs.
 *
 * Usage: stricture mysql-migrate [input_file] [-o folder] [-p prefix]
 *
 * @license MIT
...
class StrictureCommandMySQLMigrate extends libCommandLineCommand
{
	constructor(pFable, pOptions, pServiceHash)
	{
		super(pFable, pOptions, pServiceHash);

		this.options.CommandKeyword = 'mysql-migrate';
		this.options.Description = 'Generate MySQL migration stubs.';
	...
	onRunAsync(fCallback)
	{
		...
		tmpLoader.loadFromFile(tmpInputFile,
			(pError) =>
			{
				...
				let tmpGenerator = this.fable.instantiateServiceProvider('StrictureGenerateMySQLMigrate');
				tmpGenerator.generate({ OutputLocation: tmpOutputLocation, OutputFileName: tmpOutputFileName }, fCallback);
			});
	}
}

module.exports = StrictureCommandMySQLMigrate;
