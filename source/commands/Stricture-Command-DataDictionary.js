/**
 * Stricture CLI Command - Data Dictionary
 *
 * Generates LaTeX data dictionary documentation.
 *
 * Usage: stricture data-dictionary [input_file] [-o folder] [-p prefix]
 *
 * @license MIT
...
class StrictureCommandDataDictionary extends libCommandLineCommand
{
	constructor(pFable, pOptions, pServiceHash)
	{
		super(pFable, pOptions, pServiceHash);

		this.options.CommandKeyword = 'data-dictionary';
		this.options.Description = 'Generate LaTeX data dictionary documentation.';
		this.options.Aliases.push('dd');
	...
	onRunAsync(fCallback)
	{
		...
		tmpLoader.loadFromFile(tmpInputFile,
			(pError) =>
			{
				...
				let tmpGenerator = this.fable.instantiateServiceProvider('StrictureGenerateLaTeX');
				tmpGenerator.generate({ OutputLocation: tmpOutputLocation, OutputFileName: tmpOutputFileName }, fCallback);
			});
	}
}

module.exports = StrictureCommandDataDictionary;
